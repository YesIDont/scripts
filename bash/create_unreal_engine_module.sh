...
# This script automates the creation of a new Unreal Engine module within an existing Unreal Engine project.
# It performs the following tasks:
# 1. Parses the command line arguments to get the module name.
# 2. Checks if the necessary tools (like jq) are installed.
# 3. Defines the necessary paths and checks if the required project files exist.
# 4. Creates the module directories (Public and Private).
# 5. Generates the .Build.cs, header, and cpp files for the new module.
# 6. Adds the new module to the project's .uproject file.
# 7. Updates the project's Target.cs, Editor.Target.cs, and .Build.cs files to include the new module.
# Usage: ./create_unreal_engine_module.sh -n ModuleName
